...
import { sendWeeklyMessagesNow } from '../scheduler/weekly-messages.js';
...
function saveChannelsConfig(config) {
    writeFileSync(channelsConfigPath, JSON.stringify(config, null, 4));
}
...
function saveSettingsConfig(config) {
    writeFileSync(settingsConfigPath, JSON.stringify(config, null, 4));
}
